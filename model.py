...
class TrajTransformer(nn.Module):
  def __init__(self, parameters, graph=None):
    ...
    emb = nn.Sequential(
      Embeddings_traj(
        (self.loc_emb_size, self.tim_emb_size, self.tim_emb_size),
        (self.loc_size, self.tim_size, self.tim_size),
        g=graph,
      ),
      c(position),
    )
    self.model = EncoderDecoder(
      ...
      Decoder(DecoderLayer(input_size, c(attn), c(inter_attn), c(ff), parameters.dropout_p), N),
      emb,
      emb,
      Generator(input_size, self.loc_size, self.uid_size, self.uid_emb_size),
    )
