...
def generate_input_transformer(data_neural, mode, max_len=20, max_tgt=20, batch_n=50, candidate=None, graph=None):
  if candidate is None:
    candidate = list(data_neural.keys())[:100]
  batch_n = min(10, len(candidate))
  batches = []
  ...
  for uid in data_neural.keys():
    ...
    for ind in train_id:
      ...
      for j in range(0, len(locs) - 1):
        (max(locs[j], locs[j + 1]), min(locs[j], locs[j + 1]))
...
def get_acc(scores, tgt_y, tgt_len):
  ...
  for i in range(len(tgt_len)):
    score = F.log_softmax(scores[i, : tgt_len[i], :], dim=-1)
    # print(score.size())
    _val, idxx = score.data.topk(10, dim=-1)
    # print(idxx.size())
    predx = idxx.numpy()
    target = tgt_y[i]
...
def run(args):
  ...
  if args.pretrain > 0:
    checkpoint_path = os.path.join(args.save_path, 'checkpoint', f'ep_{args.pretrain}.m')
    if os.path.exists(checkpoint_path):
      print(f'Loading checkpoint from {checkpoint_path}')
      model.load_state_dict(torch.load(checkpoint_path))
      print(f'Checkpoint loaded. Continuing from epoch {args.pretrain + 1}.')
    else:
  ...
  scheduler = optim.lr_scheduler.ReduceLROnPlateau(
    optimizer, 'max', patience=parameters.lr_step, factor=parameters.lr_decay, threshold=1e-3
  )
  lr = parameters.lr
  candidate = list(parameters.data_neural.keys())

  SAVE_PATH = 'D:/GCDAN-master/results/'
  tmp_path = 'checkpoint/'
  ...
  args.pretrain + 1 if args.pretrain > 0 else 0
...
if __name__ == '__main__':
  ...
  parser.add_argument('--epoch_max', type=int, default=5)
